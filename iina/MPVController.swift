...
class MPVController: NSObject {
  ...
  private func adjustCodecWhiteList() {
    ...
    guard let whitelist = getString(MPVOption.Video.hwdecCodecs) else {
      ...
      // default value.
      Logger.log("Failed to obtain the value of option \(MPVOption.Video.hwdecCodecs)", level: .error)
      return
    }
    Logger.log("Hardware decoding whitelist (\(MPVOption.Video.hwdecCodecs)) is set to \(whitelist)")
    var adjusted: [String] = []
    ...
    codecLoop: for codec in whitelist.components(separatedBy: ",") {
      ...
      Logger.log("This Mac does not support \(codec) hardware decoding")
    }
